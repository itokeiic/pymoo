...
def cross_sbx(X, xl, xu, eta, prob_var, prob_bin, eps=1.0e-14):
    ...
    # Round the first variable and ensure it stays within bounds
    X[:, :, 0] = np.clip(np.round(X[:, :, 0]), xl[0], xu[0])
    
    # Check if parents have equal values for first variable
    are_parents_equal = np.min(X[:, :, 0], axis=0) == np.max(X[:, :, 0], axis=0)
    
    # the probability of a crossover for each of the variables
    cross = np.random.random((n_matings, n_var)) < prob_var

    # Disable crossover for pairs with different first variable values
    cross[~are_parents_equal] = False
...
class SBX_drone(SimulatedBinaryCrossover):
    pass
